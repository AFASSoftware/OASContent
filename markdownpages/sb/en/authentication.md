...
When an app connects at the Admin Center level, there is one extra step in the OAuth flow to request an [`access_token`](../../apidoc/sb/en/latest#post-/authentication/getaccesstoken) from the customer environment. First, you must complete the standard consent flow. After that, an additional request follows to obtain the `access_token` for a customer environment.
...
Endpoint: [GetAccessToken](../../apidoc/sb/en/latest#post-/authentication/getaccesstoken)
