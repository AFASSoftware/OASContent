...
Profit kan vragen om een door de server bepaalde waarde (*nonce*) in de DPoP proof op te nemen. In dat geval krijg je een foutmelding `use_dpop_nonce` met de header `DPoP-Nonce`. Maak dan een nieuwe proof aan met de claim `nonce` gelijk aan die waarde en herhaal de aanvraag. `<TODO: bevestig of Profit DPoP nonces gebruikt>`
...
**Mijn tokenaanvraag geeft `invalid_client`. Wat nu?**
Controleer of de `client_id` klopt, of de client secret niet is verlopen of ingetrokken (Profit 9), of bij private key JWT de publieke key correct is geregistreerd en de claims `iss`, `sub`, `aud` en `exp` kloppen.
